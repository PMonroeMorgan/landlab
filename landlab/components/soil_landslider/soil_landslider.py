...
using exactly Benjamin Campforts BedrockLandslider but only grabbing the soil above the failure envelope...
Benjamin Campforts
...
#### again remove landslide volume bedrock
    ...
    def _landslide_erosion(self, dt):
        ...
        while critical_landslide_nodes.size > 0:
            ...
            #### trim the initial uphill neighbors to neighbors with steeper than the angle of int friction
            #### need to think if this makes sense for shallow, I think not. 
            #### 
            #neighbors_up = neighbors_up[
            #    slope_neighbors_to_crit_node > self._angle_int_frict
            #]
            ##### also trim the initial slope arr
            #slope_neighbors_to_crit_node = slope_neighbors_to_crit_node[
            #    slope_neighbors_to_crit_node > self._angle_int_frict
            #]

            #### heres where the ground failure happens
            #### identify the sliding angle as the bisector
            
            if slope_neighbors_to_crit_node.size > 0:
                ####slope slide is the angle to the uphil
